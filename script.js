...
function normalizeState(s){
  s=s||{};
  return {channels:Array.isArray(s.channels)?s.channels:SEED_DATA.channels,people:Array.isArray(s.people)?s.people:SEED_DATA.people,statuses:Array.isArray(s.statuses)?s.statuses:SEED_DATA.statuses,tasks:Array.isArray(s.tasks)?s.tasks:[],uploads:Array.isArray(s.uploads)?s.uploads:[]};
}
...
function fillSelects(){
  ['taskOwner','filterPerson','uploadBy'].forEach(id=>{let el=$('#'+id); if(!el)return; let first=id==='filterPerson'?'<option value="">كل الفريق</option>':'';el.innerHTML=first+state.people.map(p=>`<option>${safe(p)}</option>`).join('')});
...
function renderDeliveryAlerts(){ const box=$('#deliveryAlerts'); if(!box)return; if(!state.people.length){ box.innerHTML='<div class="alert-card green">لا يوجد أعضاء فريق بعد.</div>'; return; } box.innerHTML=state.people.map(p=>{ const tasks=activeTasks().filter(t=>t.owner===p); const todayTasks=tasks.filter(isDueToday); const lateTasks=tasks.filter(isLate); const cls=lateTasks.length?'red':(todayTasks.length?'orange':'green'); const reason=lateTasks.slice(0,2).map(t=>`${safe(t.title)}: ${safe(t.delayReason||'لم يكتب سبب التأخير')}`).join('<br>'); return `<div class="alert-card ${cls}"><div class="alert-name">${safe(p)}</div><div class="alert-counts"><span>اليوم: ${todayTasks.length}</span><span>متأخر: ${lateTasks.length}</span></div>${lateTasks.length?`<small>${reason}</small>`:'<small>لا توجد مشاكل تسليم.</small>'}</div>`; }).join(''); }
function renderTeam(){ const grid=$('#teamGrid'); if(!grid)return; grid.innerHTML=state.people.map(p=>{let tasks=activeTasks().filter(t=>t.owner===p);let cls=tasks.filter(isLate).length?'red':(tasks.filter(isDueToday).length?'orange':'green');return `<div class="person-card team-status-card ${cls}"><h3>${safe(p)}</h3><p>${tasks.length} تاسكات</p><span class="pill">${tasks.filter(isDone).length} تم</span><span class="pill">${tasks.filter(isDueToday).length} تسليم اليوم</span><span class="pill">${tasks.filter(isLate).length} متأخر</span><div class="person-actions"><button data-action="rename-person" data-name="${safeAttr(p)}">تعديل الاسم</button><button class="danger small-danger" data-action="remove-person" data-name="${safeAttr(p)}">حذف</button></div></div>`}).join('') || '<div class="panel">لا يوجد أعضاء فريق بعد.</div>' }
async function addPerson(){ const name=prompt('اكتب اسم عضو الفريق الجديد:'); if(!name || !name.trim()) return; if(state.people.includes(name.trim())){ alert('الاسم موجود بالفعل.'); return; } state.people.push(name.trim()); await save(); }
async function renamePerson(oldName){ const name=prompt('تعديل اسم عضو الفريق:', oldName); if(!name || !name.trim() || name.trim()===oldName) return; if(state.people.includes(name.trim())){ alert('الاسم موجود بالفعل.'); return; } state.people=state.people.map(p=>p===oldName?name.trim():p); state.tasks.forEach(t=>{ if(t.owner===oldName) t.owner=name.trim(); if(t.deliveredBy===oldName) t.deliveredBy=name.trim(); }); state.uploads.forEach(u=>{ if(u.by===oldName) u.by=name.trim(); }); await save(); }
async function removePerson(name){ const assigned=state.tasks.filter(t=>t.owner===name && !isDone(t)).length; if(assigned && !confirm(`${name} عليه ${assigned} تاسك غير منتهي. هل تريد الحذف من الفريق مع ترك التاسكات باسمه؟`)) return; if(!assigned && !confirm(`حذف ${name} من الفريق؟`)) return; state.people=state.people.filter(p=>p!==name); await save(); }
...
function openUploadDialog(){ fillSelects(); $('#uploadForm').reset(); updateUploadPrograms(); $('#uploadDialog').showModal(); }
async function saveUploadForm(e){ e.preventDefault(); const file=$('#uploadFile').files[0]; let github=null; const linkedTaskId=$('#uploadTask').value; const linkedTask=state.tasks.find(t=>t.id===linkedTaskId); const meta={channel:$('#uploadChannel').value,program:$('#uploadProgram').value,episode:$('#uploadEpisode').value,name:$('#uploadName').value}; try{github=await uploadFileDirectToGithub(file,meta)}catch(err){console.error(err);alert('تم حفظ السجل، لكن رفع الملف على GitHub لم يكتمل. اتأكد من التوكن وصلاحية Contents Read/Write.')} const item={id:crypto.randomUUID(),name:$('#uploadName').value,channel:$('#uploadChannel').value,program:$('#uploadProgram').value,episode:$('#uploadEpisode').value,by:$('#uploadBy').value,taskId:linkedTaskId,taskTitle:linkedTask?.title||'',link:$('#uploadLink').value || github?.url || '',githubPath:github?.path || '',notes:$('#uploadNotes').value,createdAt:new Date().toISOString(),createdAtText:new Date().toLocaleString('ar-EG')}; state.uploads=[item,...(state.uploads||[])]; if(linkedTask){ linkedTask.status='تم التسليم'; linkedTask.deliveredAt=item.createdAt; linkedTask.deliveredBy=item.by; linkedTask.deliveredUploadId=item.id; linkedTask.updatedAt=item.createdAt; linkedTask.delayReason=''; } $('#uploadDialog').close(); await save(); }
function renderAll(){state=normalizeState(state);renderChannels();fillSelects();renderDaily();renderCalendar();renderFlow();renderDrawer();renderTeam();renderStats();renderUploads();renderDeliveryAlerts()}
function bindEvents(){
  ...
  $('#importInput')?.addEventListener('change',e=>{let f=e.target.files[0];if(!f)return;let r=new FileReader();r.onload=async()=>{state=normalizeState(JSON.parse(r.result));await save()};r.readAsText(f)});
